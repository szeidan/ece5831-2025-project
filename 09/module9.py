...
# module9.py
# Test handwritten digit images with trained LeNet Keras model (lenet_keras.h5)
# Usage examples:
#   python module9.py 3_2.png 3
#   python module9.py test_images/7_0.png 7
...
# (copied from module5-3.py and reused)
# =============================
...
def predict_single(model, img_path: str, true_digit: int, show=True) -> bool:
    """Predict a single image and print Success/Fail line using Keras LeNet."""
    x = load_digit_image(img_path, force_resize=True)
    if show:
        show_image(x, title=f"{os.path.basename(img_path)} (expect {true_digit})")

    # Keras ConvNet expects shape (1, 28, 28, 1)
    x_in = x.reshape(1, 28, 28, 1)
    probs = model.predict(x_in, verbose=0)[0]
    pred = int(np.argmax(probs))

    if pred == int(true_digit):
        print(
            f"Success: Image {os.path.basename(img_path)} is for digit {true_digit} "
            f"and the inference result is {pred}.",
            flush=True,
        )
        return True
    else:
        print(
            f"Fail: Image {os.path.basename(img_path)} is for digit {true_digit} "
            f"but the inference result is {pred}.",
            flush=True,
        )
        return False


def predict_all_in_folder(model, folder="test_images") -> None:
    ...
    for n in range(10):
        for m in range(5):
            ...
            if not os.path.exists(path):
                continue
            ok = predict_single(model, path, true_digit=n, show=False)
            total += 1
            correct += int(ok)
    if total:
...
def main():
    parser = argparse.ArgumentParser(
        description="Test handwritten digits using trained LeNet Keras model"
    )
    ...
    parser.add_argument(
        "--model",
        default="lenet_keras.h5",
        help="Path to trained LeNet Keras model (.h5)",
    )
    parser.add_argument(
        "--all",
        action="store_true",
        help="Test all images in test_images/ (0_0.png ... 9_4.png)",
    )
    ...
    if args.all:
        ...
        matplotlib.use("Agg")
        predict_all_in_folder(model, folder="test_images")
        return
    ...
    predict_single(model, img_path, args.digit, show=True)
